...
public class SenzorRepository extends AbstractRepository<Senzor> {
    ...
    public List<Senzor> findByRoomId(int roomId) {
        String sql = "SELECT * FROM senzori WHERE room_id = ?";
        List<Senzor> list = new ArrayList<>();
        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ps.setInt(1, roomId);
            ...
            throw new AppException("Eroare la cautare senzori dupa room: " + e.getMessage());
        }
    }

    public int nextId() {
        String sql = "SELECT COALESCE(MAX(id), 0) + 1 AS next_id FROM senzori";
        try (PreparedStatement ps = getConnection().prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            return rs.next() ? rs.getInt("next_id") : 1;
        } catch (SQLException e) {
            throw new AppException("Eroare la generare id senzor: " + e.getMessage());
        }
    }
